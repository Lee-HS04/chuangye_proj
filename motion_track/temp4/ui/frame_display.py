...
def process_frame(
    ...
    # ==========================================================
    if "baseline_feet_y" not in st.session_state:
        st.session_state["baseline_feet_y"] = (
            keypoints_3d[15][1] + keypoints_3d[16][1]
        ) / 2

    baseline_feet_y = st.session_state["baseline_feet_y"]
    ...
    if "cached_cv" not in st.session_state:
        st.session_state["cached_cv"] = 0.0

    st.session_state["cached_cv"] = sway_tracker.get_cv()
    sway_cv = st.session_state["cached_cv"]
