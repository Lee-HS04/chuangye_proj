...
if "balance_tracker" not in st.session_state:
    # Keep one canonical sway tracker object for all Streamlit CV reads/writes.
    st.session_state["balance_tracker"] = st.session_state["sway_tracker"]

# Ensure both keys always reference the same object.
st.session_state["sway_tracker"] = st.session_state["balance_tracker"]
...
# Enable CV trace logs by setting environment variable CV_DEBUG=1.
st.session_state["cv_debug_enabled"] = os.getenv("CV_DEBUG", "0") == "1"
st.session_state["cv_debug_pipeline"] = "streamlit"
...
def _render_frame(cap: cv2.VideoCapture, frame_placeholder) -> bool:
    ...
    if idx == 0 and st.session_state.get("cv_debug_enabled"):
        input_fps = cap.get(cv2.CAP_PROP_FPS)
        print(
            f"[CVDEBUG][streamlit] input_meta_fps={input_fps:.6f} "
            f"tracker_fps={st.session_state['sway_tracker'].fps} "
            f"total_frames={st.session_state.get('total_frames')}"
        )
